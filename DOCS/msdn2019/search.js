...
function search() {
    ...
    for (let qtermIndex in qterms) {
        ...
        // Uncomment the following line, if you want to always match whole words, even if no quotes around are used.
        //onlyWholeWord = true;	
        ...
        if (index[term] !== undefined) {
            for (let i in index[term]) {
                let docId = index[term][i][0];
                let relevance = index[term][i][1];
        ...
        if (!onlyWholeWord) {
            for (let indexedTerm in index) {
                if (indexedTerm.indexOf(term) >= 0) {
                    for (let i in index[indexedTerm]) {
                        let docId = index[indexedTerm][i][0];
                        let relevance = index[indexedTerm][i][1];
                        if (candidates[docId] === undefined) {
                            candidates[docId] = {};
                            candidates[docId].matchBitmap = get_bitmap(qtermIndex, qterms.length);
                            candidates[docId].relevance = relevance;
