Public Class TokenizerClass
    ...
    Private Function GetAST(ByRef ProgramStr As String) As List(Of Token)
        ...
        For Each item In StrTokens
            ...
            'CAPTURE INTEGER
            If CheckToken(tok) = "INTEGER" And
                BeginCapture = False And CapturingNum = False Then
                CapturingNum = True
                BeginCapture = True
                TokenStr &= item
            Else
                If BeginCapture = True And CapturingNum = True And
                    CheckToken(tok) <> "INTEGER" Then
                    BeginCapture = False
                    CapturingNum = False
            ...
            If CheckToken(tok) = "_LETTER" And BeginCapture = False And
                CapturingTerm = False Then
                'begin capture
                CapturingTerm = True
                BeginCapture = True
                TokenStr &= item
            Else
                'end capture
                If BeginCapture = True And CapturingTerm = True And
                    tok.Name <> "_LETTER" Then
                    BeginCapture = False
                    CapturingTerm = False
                    ' TokenStr &= item
            ...
            If tok.Name IsNot Nothing And CheckToken(tok) = "NULL" And TokenAdded = False And
                BeginCapture = False Then
                AST.Add(tok)
            Else
                ' TokenStr &= item
            End If
    ...
    End Function
    ''' <summary>
    ''' Removes whiteSpace and Eof Token
    ''' </summary>
    ''' <param name="TokLst"></param>
    ''' <returns></returns>
    ...
    ''' <summary>
    ''' Checks token for a Significant token
    ''' The significant token is such that it is a part of the main function 
    ''' and needs a specific type of initiation or Termination for the token
    ''' 
    ''' </summary>
    ''' <param name="Tok">usually a token literal = LETTER / NUMBER / POINT </param>
    ''' <returns></returns>
    Private Function CheckToken(ByRef Tok As Token)
        Select Case Tok.Name
            ...
            Case "QUOTE"
                Return "QUOTE"
            Case "_LETTER"
                'A letter can be considered to be a part of a string
                If BeginCapture = True And CapturingStr = True Then
                    Return "STRING"
                Else
                End If
                'A letter could be seen to be a part of a variable name
                If BeginCapture = True And CapturingVar = True Then
                    Return "VARIABLE"
                Else
                End If
                'the letter could be some undefined element
                Return "_LETTER"
            Case "VARIABLE"
                'the start or end indicator for variable or;
                'the actual letter as a part of the variable name
                Return "VARIABLE"
            Case "NULL"
                'Actual nothing ie VOID! 
                Return "NULL"
        End Select
        Return "NULL"
    End Function
